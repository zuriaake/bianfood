...
$page_name = isset($_REQUEST['page']) ? $_REQUEST['page'] : 'prifile';
if(isset($company_pages_array[$page_name])) {
	$article_id = $company_pages_array[$page_name];
	$_REQUEST['id'] = $article_id;
}
/*------------------------------------------------------ */
//-- INPUT
/*------------------------------------------------------ */
...
if (!$smarty->is_cached('company.dwt', $cache_id))
{
    /* 文章详情 */
...
if(isset($article))
{
    $smarty->display('company.dwt', $cache_id);
    //$smarty->display('company.dwt', $cache_id);
}
else
{
